...
class PositionManager:
    ...
    def open_position(self, side: str, entry_price: float, size: float) -> None:
        ...
        with self._lock:
            ...
            logger.info(f"Opened position: {side} {size:.6f}@{fill_price}, SL={sltp['sl_price']}, TP={sltp['tp_price']}")
    ...
    def check_exit(self) -> None:
        ...
        with self._lock:
            ...
            if ids.get('sl') not in open_ids and ids.get('tp') not in open_ids:
                ...
                # self.active sera None si exit réussie; sinon maintenue pour indiquer un risque
                return
